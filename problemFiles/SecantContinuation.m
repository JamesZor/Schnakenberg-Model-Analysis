function branch = SecantContinuation(problem,u0,p0,stepperPars)
   ...
   function DisplayStep(step,u,p,status)
     ...
     if numBranchVariables == 0
       ...
       temp =BranchVariables(step,u,p);
       outString = sprintf(formatString, step, numUnstableEigenvalues, p(iContPar), norm(u), temp(1),temp(101));
     end
     fprintf(outString);
